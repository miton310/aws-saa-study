...
import VPCIntermediateStepper from '@/components/visualizations/VPCIntermediateStepper'
import QuizSection from '@/components/QuizSection'

export default function VPCIntermediatePage() {
  const cat = getCategoryBySlug('vpc')!
  const diff = getDifficultyBySlug('intermediate')!
  const content = getContent('vpc', 'intermediate')!
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10">
      ...
      <div className="flex items-center justify-between border-t border-gray-200 pt-6">
        <Link href="/vpc/beginner" className="text-gray-500 hover:text-gray-700 text-sm font-medium">
          ← 初級に戻る
        </Link>
        <Link
          href="/vpc/advanced"
