...
import VPCBeginnerStepper from '@/components/visualizations/VPCBeginnerStepper'
import QuizSection from '@/components/QuizSection'

export default function VPCBeginnerPage() {
  const cat = getCategoryBySlug('vpc')!
  const diff = getDifficultyBySlug('beginner')!
  const content = getContent('vpc', 'beginner')!
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10">
      ...
      <nav className="text-sm text-gray-500 mb-6 flex items-center gap-2">
        ...
        <span>/</span>
        <Link href="/vpc" className="hover:text-gray-700">{cat.name}</Link>
        <span>/</span>
      ...
      <div className="flex items-center justify-between border-t border-gray-200 pt-6">
        <Link href="/vpc" className="text-gray-500 hover:text-gray-700 text-sm font-medium">
          ← VPC 難易度選択に戻る
        </Link>
        <Link
          href="/vpc/intermediate"
