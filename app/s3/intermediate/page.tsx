...
import S3IntermediateStepper from '@/components/visualizations/S3IntermediateStepper'
import QuizSection from '@/components/QuizSection'

export default function S3IntermediatePage() {
  const cat     = getCategoryBySlug('s3')!
  const diff    = getDifficultyBySlug('intermediate')!
  const content = getContent('s3', 'intermediate')!
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10">
      ...
      <div className="flex items-center justify-between border-t border-gray-200 pt-6">
        <Link href="/s3/beginner" className="text-gray-500 hover:text-gray-700 text-sm font-medium">
          ← 初級に戻る
        </Link>
        <Link
          href="/s3/advanced"
