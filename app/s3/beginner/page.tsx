...
import S3BeginnerStepper from '@/components/visualizations/S3BeginnerStepper'
import QuizSection from '@/components/QuizSection'

export default function S3BeginnerPage() {
  const cat     = getCategoryBySlug('s3')!
  const diff    = getDifficultyBySlug('beginner')!
  const content = getContent('s3', 'beginner')!
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10">
      ...
      <nav className="text-sm text-gray-500 mb-6 flex items-center gap-2">
        ...
        <span>/</span>
        <Link href="/s3" className="hover:text-gray-700">{cat.name}</Link>
        <span>/</span>
      ...
      <div className="flex items-center justify-between border-t border-gray-200 pt-6">
        <Link href="/s3" className="text-gray-500 hover:text-gray-700 text-sm font-medium">
          S3トップへ
        </Link>
        <Link
          href="/s3/intermediate"
