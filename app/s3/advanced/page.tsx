...
import S3AdvancedStepper from '@/components/visualizations/S3AdvancedStepper'
import QuizSection from '@/components/QuizSection'

export default function S3AdvancedPage() {
  const cat     = getCategoryBySlug('s3')!
  const diff    = getDifficultyBySlug('advanced')!
  const content = getContent('s3', 'advanced')!
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10">
      ...
      <div className="flex items-center justify-between border-t border-gray-200 pt-6">
        <Link href="/s3/intermediate" className="text-gray-500 hover:text-gray-700 text-sm font-medium">
          ← 中級に戻る
        </Link>
        <Link
          href="/s3"
