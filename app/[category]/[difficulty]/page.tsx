import Link from 'next/link'
import { notFound } from 'next/navigation'
import { getCategoryBySlug, getDifficultyBySlug, DIFFICULTIES } from '@/lib/categories'
import { getContent } from '@/lib/content'
import QuizSection from '@/components/QuizSection'
import DiagramRenderer from '@/components/DiagramRenderer'

export default async function LearningPage({
  params,
}: {
  params: Promise<{ category: string; difficulty: string }>
}) {
  const { category, difficulty } = await params
  const cat = getCategoryBySlug(category)
  const diff = getDifficultyBySlug(difficulty)
  const content = getContent(category, difficulty)

  if (!cat || !diff || !content) notFound()

  const currentDiffIdx = DIFFICULTIES.findIndex((d) => d.slug === difficulty)

  return (
    <div className="max-w-4xl mx-auto px-4 py-10">
      {/* Breadcrumb */}
      <nav className="text-sm text-gray-500 mb-6 flex items-center gap-2 flex-wrap">
        <Link href="/" className="hover:text-gray-700">ホーム</Link>
        <span>/</span>
        <Link href={`/${cat.slug}`} className="hover:text-gray-700">{cat.name}</Link>
        <span>/</span>
        <span className={`font-semibold ${diff.color}`}>{diff.label}</span>
      </nav>

      {/* Header */}
      <div className={`${cat.bgColor} rounded-3xl p-6 mb-8 flex items-start gap-4`}>
        <div className="text-5xl">{cat.icon}</div>
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            <h1 className={`text-2xl font-extrabold ${cat.color}`}>{cat.name}</h1>
            <span className={`text-xs font-bold px-3 py-1 rounded-full ${diff.bg} ${diff.color} border ${diff.border}`}>
              {diff.label}
            </span>
          </div>
          <p className="text-gray-600 text-sm leading-relaxed">{content.overview}</p>
        </div>
      </div>

      {/* Difficulty navigation */}
      <div className="flex gap-2 mb-8">
        {DIFFICULTIES.map((d) => (
          <Link
            key={d.slug}
            href={`/${cat.slug}/${d.slug}`}
            className={`flex-1 text-center py-2 px-3 rounded-full text-sm font-semibold border-2 transition-all ${
              d.slug === difficulty
                ? `${d.bg} ${d.color} ${d.border}`
                : 'bg-white text-gray-500 border-gray-200 hover:border-gray-300'
            }`}
          >
            {d.label}
          </Link>
        ))}
      </div>

      {/* Diagram (client component) */}
      <DiagramRenderer category={category} />

      {/* Learning sections */}
      <div className="mb-10 space-y-6">
        <h2 className="text-xl font-bold text-gray-800">📚 学習内容</h2>
        {content.sections.map((section, i) => (
          <div key={i} className="bg-white rounded-2xl border border-gray-200 p-6">
            <h3 className={`text-lg font-bold mb-3 ${cat.color} flex items-center gap-2`}>
              <span className="w-6 h-6 rounded-full bg-orange-100 text-orange-600 flex items-center justify-center text-xs font-bold shrink-0">
                {i + 1}
              </span>
              {section.title}
            </h3>
            <p className="text-gray-700 text-sm leading-relaxed mb-4">{section.body}</p>
            <ul className="space-y-2">
              {section.keyPoints.map((kp, j) => (
                <li key={j} className="flex items-start gap-2 text-sm text-gray-700">
                  <span className={`mt-0.5 shrink-0 w-4 h-4 rounded-full ${cat.bgColor} ${cat.color} flex items-center justify-center text-[10px] font-bold`}>
                    ✓
                  </span>
                  {kp}
                </li>
              ))}
            </ul>
          </div>
        ))}
      </div>

      {/* Quiz */}
      <div className="mb-10">
        <QuizSection questions={content.quiz} />
      </div>

      {/* Affiliate / Books */}
      {content.affiliateLinks.length > 0 && (
        <div className="mb-10">
          <h2 className="text-xl font-bold text-gray-800 mb-4">📚 おすすめ参考書</h2>
          <p className="text-xs text-gray-500 mb-3">
            ※本ページのリンクにはアフィリエイト広告が含まれます。
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {content.affiliateLinks.map((link) => (
              <a
                key={link.url}
                href={link.url}
                target="_blank"
                rel="noopener noreferrer"
                className="block bg-white border-2 border-gray-200 hover:border-orange-300 hover:shadow-md transition-all rounded-xl p-4"
              >
                <span className="inline-block text-xs bg-orange-500 text-white px-2 py-0.5 rounded-full mb-2 font-medium">
                  {link.badge}
                </span>
                <div className="font-semibold text-sm text-gray-800 mb-1">{link.title}</div>
                <div className="text-gray-500 text-xs leading-relaxed mb-2">{link.description}</div>
                <div className="text-orange-500 text-xs font-semibold">Amazonで確認する →</div>
              </a>
            ))}
          </div>
        </div>
      )}

      {/* Navigation */}
      <div className="flex items-center justify-between border-t border-gray-200 pt-6">
        <Link href={`/${cat.slug}`} className="text-gray-500 hover:text-gray-700 text-sm font-medium">
          ← {cat.name} 難易度選択に戻る
        </Link>
        {currentDiffIdx < DIFFICULTIES.length - 1 && (
          <Link
            href={`/${cat.slug}/${DIFFICULTIES[currentDiffIdx + 1].slug}`}
            className="bg-orange-500 hover:bg-orange-600 text-white font-semibold px-5 py-2 rounded-full text-sm transition-colors"
          >
            {DIFFICULTIES[currentDiffIdx + 1].label}へ進む →
          </Link>
        )}
      </div>
    </div>
  )
}
